...
type TransactionProps = {
  ...
  renderMlBadge?: (transaction: TransactionEntity) => React.ReactNode;
  onApplyMlSuggestion?: (
    transaction: TransactionEntity,
    categoryId: string,
  ) => Promise<void>;
...
const Transaction = memo(function Transaction({
  ...
  const runningBalance = !isTemporaryId(id) ? balance : balance + amount;
    ...
    const mlPrediction = mlPredictionsById?.[transaction.id] ?? null;
  const mlTopCategories = getMlTopCategories?.(transaction.id) ?? [];
  const mlTop1CategoryId =
    mlTopCategories[0]?.category_id ?? mlPrediction?.predicted_category_id;

  const canShowMlSuggestion =
    !isPreview &&
    !isParent &&
    !isBudgetTransfer &&
    !isOffBudget &&
          ...
          <CustomCell
            ...
            unexposedContent={props => (
              ...
              >
                ...
                {renderMlBadge?.(originalTransaction)}
                ...
                {canShowMlSuggestion ? (
                  <button
                    ...
                    title={
                      mlPrediction
                        ? `Apply AI suggestion (${Math.round(
...
function TransactionTableInner({
  ...
  const renderRow: TableProps<TransactionEntity>['renderItem'] = ({
    ...
    mlPredictionsById={props.mlPredictionsById}
    getMlTopCategories={props.getMlTopCategories}
    renderMlBadge={props.renderMlBadge}
