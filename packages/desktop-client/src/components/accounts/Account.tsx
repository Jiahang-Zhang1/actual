...
type AccountInternalState = {
  ...
  mlPredictionsById: Record<string, MlPrediction | null>;
...
applySuggestedCategory = async (
  ...
  suggestedCategoryValue: string,
) => {
  const resolvedCategoryId =
  this.resolveSuggestedCategoryId(suggestedCategoryValue, transaction);

  console.log('applySuggestedCategory:', {
  ...
  if (!resolvedCategoryId) {
    console.error(
      'Could not map ML suggestion to an Actual category id:',
      suggestedCategoryValue,
    );
    return;
  }
  ...
  try {
    await send('transactions-batch-update', {
      updated: [{ id: transaction.id, category: resolvedCategoryId }],
    });

    const feedbackResult = await send('ml-record-feedback', {
  transactionId: transaction.id,
  finalCategoryId: resolvedCategoryId,
});

console.log('ml-record-feedback result:', feedbackResult);

    await this.refetchTransactions();

    // 重新拉一次 prediction，避免按钮状态滞后
    await this.refreshMlPredictions(this.state.transactions);
  } catch (e) {
    console.error('applySuggestedCategory failed', e);
  }
};
...
resolveSuggestedCategoryId = (
  ...
  const allCategories = this.props.categoryGroups.flatMap(
    group => group.categories || [],
  );

  const normalized = suggestedValue.trim().toLowerCase();

  // 1) 如果模型直接返回的就是 Actual category id
  const byId = allCategories.find(cat => cat.id === suggestedValue);
  if (byId) {
    return byId.id;
  }

  // 2) 精确名称匹配
  const byExactName = allCategories.find(
  ...
  // 3) 模型标签 -> App 分类名
  const labelToActualName: Record<string, string> = {
    'food & dining': 'Food',
  ...
  const mappedName = labelToActualName[normalized];
  if (mappedName) {
    const mappedCategory = allCategories.find(
      cat => (cat.name || '').trim().toLowerCase() === mappedName.toLowerCase(),
    );
    if (mappedCategory) {
  ...
  const aliasTargets = aliasMap[normalized] ?? [];
  for (const targetName of aliasTargets) {
    const match = allCategories.find(
      cat =>
        (cat.name || '').trim().toLowerCase() === targetName.toLowerCase(),
    );
    if (match) {
      return match.id;
    }
  ...
  renderMlBadge={(transaction: TransactionEntity) => {
    ...
    console.log('renderMlBadge:', transaction.id, prediction);
