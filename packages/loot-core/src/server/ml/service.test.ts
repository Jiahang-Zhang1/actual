...
describe('ml service request shaping', () => {
  ...
  it('predictCategory sends a fallback description for sparse manual entries', async () => {
    ...
    expect(body.merchant_text).toBeNull();
    expect(fetchMock.mock.calls[0][0]).toBe('http://localhost:8000/predict');
  });

  it('predictCategory preserves the visible payee as merchant_text', async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      jsonResponse({
        predicted_category_id: 'Food & Dining',
        confidence: 0.6,
        top_categories: [
          ...
          { category_id: 'Income', score: 0.25 },
          { category_id: 'Financial Services', score: 0.15 },
        ],
        model_version: 'v-payee',
      }),
    );
    vi.stubGlobal('fetch', fetchMock);

    await predictCategory({
      transactionDescription: 'STARBUCKS NO NOTES',
      notes: '',
      amount: -7.5,
    });
    ...
    const [, init] = fetchMock.mock.calls[0];
    const body = JSON.parse(String(init?.body));
    expect(body.transaction_description).toBe('STARBUCKS NO NOTES');
    expect(body.merchant_text).toBe('STARBUCKS NO NOTES');
  });
  ...
  it('predictCategory uses the same-origin smartcat proxy in browser production builds', async () => {
    ...
    try {
      ...
      const { predictCategory: predictCategoryInBrowser } =
